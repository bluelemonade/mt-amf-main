...
<template>
  ...
  <div v-if="(( content!= '') && (isMainMenuOpen==false))">
      ...
      <div class="grid-container">
        ...
        <!-- <div class="grid-header" id="fundorte"> -->
        ...
        <div class="grid-footer">
          <div @mousedown="prevFundstelle" class="footerleft">
              <p v-if="(parseInt($route.params.id)-1) > 0">&lt; {{ content.content.backfundstelle[getLanguageID] }} </p>
          </div>
          <div @mousedown="toTop" class="footermiddle">
            <p> ⌃ </p> 
          </div>
          <div class="footerright">
            <p v-if="(parseInt($route.params.id)-1) < content['fundorte-de'].length-1" @mousedown="nextFundstelle">  {{ content.content.forwardfundstelle[getLanguageID] }}  &gt;</p>
          </div>
        </div>
...
export default {
  ...
  computed: {
    // nextFundstelleText(){
    //   console.log("nextFundstelle");
    // },
    // prevFundstelleText(){
    //   console.log("prevFundstelle");
    // },
    // isMainMenuOpen () {
  ...
  methods:{
    onOpenFundort(fundOrtNr){
      ...
      // this.$router.push({ name: 'fundorte', params: { id: fundOrtNr }, hash: 'fundorte' }).catch(() => {});
      location.hash = "-";
      setTimeout( () => location.hash = "#fundorte" + this.idNr, 800);
      // setTimeout( () => location.hash = "#fundorte", 600);
      // var self = this;
      // setTimeout(
      //   function(){ 
    ...
    nextFundstelle(){
      console.log("nextFundstelle");
      this.onOpenFundort(parseInt(this.$route.params.id) +1 );
    },
    prevFundstelle(){
      console.log("prevFundstelle");
      this.onOpenFundort(parseInt(this.$route.params.id) -1);
    ...
    toTop(){
      console.log("toTop");
      this.isForscherVisible = false;
      // this.$router.push({ name: 'fundorte', params: { id: this.$route.params.id }, hash: '#window-content-' + this.idNr }).catch(() => {});
      location.hash = "-";
    ...
    showForscher: function(event){
        ...
        this.forscherX = event.offsetX +20 ; // event.layerX - shiftX;
        this.forscherY = (event.screenY * -1) + event.clientY; // event.screenY + event.pageY + event.offsetY // - event.screenY; // event.layerY - shiftY;
        this.forscherY = event.target.offsetTop +20; //event.clientY - (event.screenY * -1) + event.clientY; // event.screenY + event.pageY + event.offsetY // - event.screenY; // event.layerY - shiftY;
        // console.log("forschery", this.forscherY );
        // console.log("clientY", event.clientY, "layerY", event.layerY, "pageY", event.pageY, "offsetY", event.offsetY, "screenY", event.screenY, "x", event.x, "y", event.y);
...
.grid-container {
  ...
  grid-template-rows: 66px auto auto 64px; // 900px
  gap: 32px 32px;
  grid-template-areas:
    ...
    // "grid-footernav grid-footernav grid-footernav grid-footernav"
