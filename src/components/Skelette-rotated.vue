...
<template>
    <div>
      ...
      <!-- <transition-group tag="div" class="img-slider" name="slide"  mode="out-in" appear> -->
        <div v-for="number in [nr]" v-bind:key="number" >
          <img :src="imagelist[Math.abs(nr) % imagelist.length]"/>
          <!-- <img @click="nextSkelett" :src="imagelist[Math.abs(nr) % imagelist.length]"/> -->
        </div>
      </transition-group>
...
export default {
  data: function() {
      return {
        ...
        lastevent: 0,
        imagelist: [
                    require('@/assets/bg-skelett/skelett-1-rotated.png'),
                    require('@/assets/bg-skelett/skelett-2-rotated.png'),
                    require('@/assets/bg-skelett/skelett-3-rotated.png'),
                    require('@/assets/bg-skelett/skelett-4-rotated.png')
                    ],
        namen: { 
