    ...
    <div id="svg-container"  @click="passEvent" v-show="isIconLayerActive == true">
        <svg version="1.1" ref="iconlayer" id="wiggles" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 2840 2160" style="enable-background:new 0 0 2840 2160;" xml:space="preserve">
            ...
            <g id="group4" transform="translate(1920,1080) rotate(0)">
                ...
                <path class="blob2 circlepulse" d="M15.8,76.4c0.3-37.7,43-19.5,79.6-35.2s69.8,2,80.7,51.1c10.9,49.1-23,54.1-71.5,70.9S15.6,114.1,15.8,76.4z"  transform="translate(-90 -150) scale(1.4)"/>
                <image xlink:href="../assets/icons/stammbaum-des-lebens-icon.png" x="0" y="0" height="180px" width="180px" transform="translate(-150,-150) scale(1.5)"></image>
                <text x="-170" y="105" id="t41" text-anchor="left" class="icon-text-big"> yyy </text>
                <text x="-170" y="145" id="t42" text-anchor="left" class="icon-text-small"> xxx  </text>
            ...
            <g id="group7" transform="translate(1920,1080) rotate(0)">
                ...
                <image xlink:href="../assets/icons/quiz-icon.png" x="0" y="0" height="200px" width="200px" transform="translate(-150,-100)"></image>
                <text x="-150" y="120" id="t71" text-anchor="left" class="icon-text-big"> yyy </text>
                <text x="-150" y="160" id="t72" text-anchor="left" class="icon-text-small"> xxx  </text>
...
computed: {
    ...
    watchIcons () {
        ...
        const iconsString = this.$store.getters.getOpenWindowsStartMenu;
        return iconsString
    },
    watchImpressum () {
        // const iconsString = this.$store.getters.getOpenWindowsMenu;
        return  this.$store.getters.getOpenImpressum;
        // iconsString
    }
},
methods: {
    ...
    clickIcon (e){
        ...
        if (this.$store.getters.getWindowCanOpen == true ){
            ...
            var nextWindow = this.$store.getters.nextWindowCanOpen;
            ...
            EventBus.$emit('open-window',[nextWindow, rot, e.offsetX -150,posY ])
            // xxx neuer punkt berechnen...??? 
    ...
    clickImpressum (e){
        ...
        const impressumID = groupID.replace('#impressum','');
        console.log("impressumID", impressumID);

        if (this.$store.getters.getImpressumCanOpen == true ){
            //this.$store.dispatch('setActiveWindow', iconID); 
            this.$store.commit('setActiveImpressumM', impressumID);
            // xxx neuer punkt berechnen...??? 
            gsap.to("#impressum1", 0.5, {opacity: 0.0}); 
            gsap.to("#impressum2", 0.5, {opacity: 0.0}); 
            // EventBus.$emit('custom-event');
            EventBus.$emit('open-impressum', { "impressumID": impressumID })
        }

        // eventHub.$emit('open-impressum', { "impressumID": impressumID })
    ...
    randomMovement(element, arrayPos) { 
        ...
        gsap.to(element, this.random(1.5, 2.5), {x: newPoint[0], y: newPoint[1], rotation: drehung,  delay: this.random(15, 60), ease:"power2.inout", onComplete:this.randomMovement, onCompleteParams:[element, arrayPos]});
...
watch: {
    ...
    watchImpressum (newImpressum, oldImpressum) {
        console.log("watchImpressum", newImpressum, oldImpressum);

        // step durch alle icons und einblenden
        if (newImpressum == true){
            gsap.to("#impressum1", 0.5, {opacity: 1.0}); 
            gsap.to("#impressum2", 0.5, {opacity: 1.0}); 
        }
    }
}
    // getBigText () {
